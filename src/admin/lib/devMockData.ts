...
/**
 * Demo collection data for the admin dev-mock mode (see devMock.ts for the
 * flag and safety notes). Split from devMock.ts because building these rows
 * needs the Firestore `Timestamp` class as a runtime value — and devMock.ts is
 * reachable from the always-mounted AdminContext, which must stay free of
 * Firebase imports so the SDK never enters the initial bundle (issue #234).
 * This module is only imported by the lazily-loaded admin data hooks.
 */
